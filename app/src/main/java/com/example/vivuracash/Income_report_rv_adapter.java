...
public class Income_report_rv_adapter extends RecyclerView.Adapter<Income_report_rv_adapter.ViewHolder> {
    private List<Income_Personal_Model> MY_Income_list;
    private Context ctx;


    public Income_report_rv_adapter(List<Income_Personal_Model> MY_Income_list,Context ctx){
        this.MY_Income_list=MY_Income_list;
        this.ctx=ctx;
    }

    public void filterList(ArrayList<Income_Personal_Model> filterllist) {
    ...
    @NonNull
    @Override
    public Income_report_rv_adapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View myView = LayoutInflater.from(parent.getContext()).inflate(R.layout.persoanl_income_rv__layout,parent,false);
        ViewHolder holder= new ViewHolder(myView);
        return holder;
    }

    @Override
    public void onBindViewHolder(@NonNull Income_report_rv_adapter.ViewHolder holder, int position) {
